...
st.markdown("""
    ...
    @media (prefers-color-scheme: dark) {
        :root {
            ...
            --accent-color: #00FF00; /* Bright Lime */
            --card-bg: #8B008B; /* Dark Magenta */
            --border-color: #FFFF00; /* Bright Yellow */
        }
    }
...
# Replacing yfinance stock info function with a mock version
@st.cache_data(ttl=86400)  # Cache for 24 hours
def get_stock_info(ticker):
    """Get stock information without using yfinance"""
    try:
        ...
        if response.status_code == 200:
            ...
            if data and len(data) > 0:
                ...
                return {
                    ...
                    "dividend_yield": stock_data.get('lastDiv', 0) * 100
                }
        
        # If API call fails, return default data based on ticker
        # This is a fallback mechanism using predefined mock data
        mock_data = {
            "INFY.NS": {"name": "Infosys Ltd", "sector": "Technology", "price": 1500.0, "beta": 0.9},
            "HDFCBANK.NS": {"name": "HDFC Bank", "sector": "Financial Services", "price": 1650.0, "beta": 1.1},
            "TCS.NS": {"name": "Tata Consultancy Services", "sector": "Technology", "price": 3300.0, "beta": 0.8},
            "RELIANCE.NS": {"name": "Reliance Industries", "sector": "Energy", "price": 2500.0, "beta": 1.2},
            "SUNPHARMA.NS": {"name": "Sun Pharmaceutical", "sector": "Healthcare", "price": 1100.0, "beta": 0.7}
        }
        
        # Find the ticker or return default
        ticker_data = mock_data.get(ticker, {"name": ticker, "sector": "Unknown", "price": 1000.0, "beta": 1.0})
...
# Replace yfinance historical data function with mock data generator
@st.cache_data(ttl=43200)  # Cache for 12 hours
def get_historical_data(tickers, period="5y"):
    """Generate mock historical stock data without using yfinance"""
    if not tickers:
        return pd.DataFrame()
    
    try:
        # Generate date range covering the requested period
        if period == "5y":
            days = 365 * 5
        elif period == "1y":
            days = 365
        ...
        # Generate realistic price data for each ticker
        for ticker in tickers:
            # Get base price from mock data or use default
            mock_data = {
                "INFY.NS": 1500.0,
                "TCS.NS": 3300.0,
                "HDFCBANK.NS": 1650.0,
                ...
                "CIPLA.NS": 1150.0,
                "DIVISLAB.NS": 3600.0,
                "APOLLOHOSP.NS": 5300.0,
                "ONGC.NS": 200.0,
                "IOC.NS": 140.0,
                "NTPC.NS": 260.0,
                "POWERGRID.NS": 240.0
            }
            base_price = mock_data.get(ticker, 1000.0)
            
            # Volatility and trend parameters - can be customized per ticker
            if "TECH" in ticker or "INFO" in ticker:
                volatility = 0.015  # 1.5% daily volatility for tech stocks
                trend = 0.0002  # Slight upward trend
            elif "BANK" in ticker or "FIN" in ticker:
                volatility = 0.012  # 1.2% daily volatility for financial stocks
                trend = 0.0001  # Minimal trend
            else:
                volatility = 0.01  # 1% daily volatility for others
                trend = 0.00005  # Very slight trend
            
            # Generate random walk prices
            np.random.seed(hash(ticker) % 10000)  # Seed based on ticker name for consistency
            price_changes = np.random.normal(trend, volatility, len(date_range))
            price_changes[0] = 0  # Start with no change
            
            # Calculate cumulative changes and convert to prices
        ...
        st.warning(f"Error generating historical data: {e}")
        return pd.DataFrame()
